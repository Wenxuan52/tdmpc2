#!/usr/bin/env python3
"""Plot MyoSuite single-task curves with mean and 95% CI over 3 seeds."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("/root/workspace/tdmpc2/figures/MyoSuite.pdf"),
        help="Output pdf path.",
    )
    parser.add_argument(
        "--seed-config",
        type=Path,
        default=Path("tools/myosuite/seed.yaml"),
...
def plot_all(args: argparse.Namespace) -> None:
    ...
    ours_seed_config = load_ours_seed_config(args.seed_config, MYOSUITE_TASKS)
    ...
    legend_handles = []
    for idx, task in enumerate(MYOSUITE_TASKS):
        ax = axes[idx]
