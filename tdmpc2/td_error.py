...
def main():
	parser = argparse.ArgumentParser(description='Launch TD-error metric parallel training jobs.')
	default_cfg = Path(__file__).resolve().with_name('td_error.yaml')
	parser.add_argument('--config', default=str(default_cfg), help='Path to td-error parallel config yaml.')
