...
def main(cfg):
    ...
    train_steps_per_iter = int(getattr(cfg, 'train_steps_per_iter', 1000) or 1000)
    if train_steps_per_iter <= 0:
        raise ValueError('train_steps_per_iter must be > 0')

    def train_for_steps(num_steps: int):
        nonlocal step, obs, done, tds
        ran_updates = 0
    ...
    # Build buffer / pass seed stage first, then warmup planning+updates.
    train_for_steps(int(cfg.seed_steps) + 10)

    for _ in range(warmup_evals):
        train_for_steps(train_steps_per_iter)
