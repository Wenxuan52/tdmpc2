...
class DiffusionPlanner:
	...
	def _estimate_G(self, agent, z0, actions, task):
		...
		_, info = agent.model.pi(z, task)
		q_bootstrap = agent.model.Q(z, info["mean"], task, return_type='avg')
		G = G + discount * alive * q_bootstrap
		return G.squeeze(-1)

	@torch._dynamo.disable
	def _compute_mf_score(self, agent, z0, a0_samples, alpha_bar_tau, task):
		...
		return weighted_grad.sum(dim=0) / (torch.sqrt(alpha_bar_tau) + 1e-8)

	def _plan_impl(self, agent, obs, alphas, alpha_bar, t0=False, eval_mode=False, task=None):
		cfg = self.cfg
		device = agent.device
		...
		for tau in range(self._num_steps - 1, 0, -1):
			...
			if use_mf:
				score_mf = self._compute_mf_score(agent, z0, a0_samples, alpha_bar_tau, task)
	...
	def plan(self, agent, obs, t0=False, eval_mode=False, task=None):
		alphas, alpha_bar = self._get_schedule(agent.device)
		if self._compile and hasattr(torch, 'compiler') and hasattr(torch.compiler, 'cudagraph_mark_step_begin'):
			torch.compiler.cudagraph_mark_step_begin()
			alphas = alphas.clone()
			alpha_bar = alpha_bar.clone()
		return self._plan_fn(agent, obs, alphas, alpha_bar, t0=t0, eval_mode=eval_mode, task=task)
